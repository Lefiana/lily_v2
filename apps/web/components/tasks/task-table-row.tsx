...
import { ITaskWithSubtasks, TaskStatus, TaskCategory } from "../../types/task";
  ...
  const handleDeleteFile = async (attachmentId: string) => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/tasks/attachments/${attachmentId}`,
        {
          method: "DELETE",
          credentials: "include",
        },
        ...
        <div className="flex items-center gap-4 px-4 py-4">
          <CollapsibleTrigger asChild>
            <Button
              ...
              aria-label={
                isOpen ? "Collapse quest details" : "Expand quest details"
              }
            >
              <ChevronRight
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3">
              <span
                className={`font-medium truncate text-zinc-100 ${task.status === TaskStatus.COMPLETED ? "line-through text-zinc-500" : ""}`}
              >
                {task.title}
              </span>
              <Badge
                variant="outline"
                className="text-[10px] border-white/20 text-zinc-400 uppercase font-bold px-1.5 h-5"
              >
                {task.status}
              </Badge>
              {task.category && task.category !== TaskCategory.TASK && (
                <Badge
                  variant="outline"
                  className="text-[10px] border-purple-500/40 text-purple-400 uppercase font-bold px-1.5 h-5"
                >
                  {task.category}
                </Badge>
              )}
          ...
          <div className="px-16 pb-8 pt-2 grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="space-y-3">
                ...
                {isAddingSubtask ? (
                  <div className="flex items-center gap-2 pt-2">
                    <label htmlFor={`new-obj-${task.id}`} className="sr-only">
                      New Objective
                    </label>
                    <input
                      id={`new-obj-${task.id}`}
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="bg-white/[0.02] border border-white/5 rounded-lg p-4">
                  <p className="text-sm text-zinc-400 italic leading-relaxed">
                    {task.description ||
                      "No further details available for this quest."}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {(task.TaskAttachment || []).map((file) => {
                      ...
                      return (
                        <div
                          key={file.id}
                          className="group relative aspect-square bg-white/5 border border-white/10 rounded-lg overflow-hidden transition-all hover:border-white/20"
                        >
                          {/* Preview logic */}
                          ...
                          <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-8 w-8 p-0 rounded-full bg-red-500/20 hover:bg-red-500/40"
                ...
                {/* Upload Action */}
                <div className="pt-2">
                  <label htmlFor={`file-input-${task.id}`} className="sr-only">
                    Upload quest intel files
                  </label>
                  <input
                    type="file"
