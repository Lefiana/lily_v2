...
import { TaskEvents, ISubtaskAddedPayload, ITaskAttachment } from '../domain/task.types';
...
export class TasksService {
  ...
  async uploadFile(taskId: string, userId: string, file: Express.Multer.File) {
    ...
    try {
        ...
        this.logger.warn(`File not found on disk, but DB record deleted: ${fullPath}`);
        }
    ...
    async uploadMultipleFiles(taskId: string, userId: string, files: Express.Multer.File[]) {
    try {
        await this.findOne(taskId, userId);
...
async getAttachmentForDownload(attachmentId: string, userId: string) {
  ...
  async updateSubtask(subtaskId: string, dto: UpdateSubtaskDto, userId: string) {
    try {
  ...
  async deleteSubtask(subtaskId: string, userId: string) {
    try {
      ...
      this.gateway.emitToUser(userId, TaskEvents.SUBTASK_DELETED, { id: subtaskId });
      return { success: true };
    } catch (error) {
